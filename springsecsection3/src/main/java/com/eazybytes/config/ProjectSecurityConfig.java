...
public class ProjectSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		CustomerJDBCUserDetails customerJDBCUserDetails =  new CustomerJDBCUserDetails();
		Customer customer = new Customer();
		customer.setEmail("mabaloch@gmail.com");
		customer.setPwd("abc");
		customer.setRole("SuperUser");
		SecurityCustomer securityCustomer = new SecurityCustomer(customer);
		customerJDBCUserDetails.createUser(securityCustomer);
		auth.userDetailsService(customerJDBCUserDetails);
	}
